...
//header file for imu
#include "mecanum_msgs/Imu.h"

#include "mecanum_base_config.h"
#include "Motor.h"
...
Kinematics kinematics(Kinematics::MECANUM_BASE, MAX_RPM, WHEEL_DIAMETER, FR_WHEELS_DISTANCE, LR_WHEELS_DISTANCE);
...
void PIDCallback(const mecanum_msgs::PID& pid);

ros::NodeHandle nh;

ros::Subscriber<geometry_msgs::Twist> cmd_sub("cmd_vel", commandCallback);
ros::Subscriber<mecanum_msgs::PID> pid_sub("pid", PIDCallback);

mecanum_msgs::Imu raw_imu_msg;
ros::Publisher raw_imu_pub("raw_imu", &raw_imu_msg);

mecanum_msgs::Velocities raw_vel_msg;
...
void setup()
{
    ...
    nh.loginfo("MECANUMBASE CONNECTED");
    delay(1);
}
...
void PIDCallback(const mecanum_msgs::PID& pid)
{
    //callback function every time PID constants are received from mecanum_pid for tuning
